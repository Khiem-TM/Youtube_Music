...
export default function PlaylistPicker() {
  ...
  const [pending, setPending] = useState(null); // { refId, type, ...meta }
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    await localSvc.addToPlaylist(
                      pl._id,
                      pending.refId,
                      payload
                    );
                    window.dispatchEvent(
                      new CustomEvent("playlist-item-added", {
                        detail: { refId: pending.refId, playlistId: pl._id },
                      })
                    );
                    setOpen(false);
                  } catch (e) {
                    if (e?.response?.status === 409) {
                      window.dispatchEvent(
                        new CustomEvent("playlist-item-duplicate", {
                          detail: { refId: pending.refId, playlistId: pl._id },
                        })
                      );
                      setOpen(false);
            ...
            {creating ? (
              <div className="space-y-2">
                <input
                  ...
                  className="w-full bg-dark-800 border border-gray-700 rounded px-3 py-2 text-white"
                />
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        createdPlaylist = await localSvc.createPlaylist(name);
                    ...
                    className="px-3 py-2 rounded bg-primary-500 text-white"
                  >
                    Tạo
                  </button>
                </div>
