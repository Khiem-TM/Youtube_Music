...
export default function PlaylistDetails() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await axios.get(`${BASE_URL}/playlists/details/${slug}`, { params: { limit: 200 } });
        const d = res.data || {};
        setData(d);
  ...
  const playAll = () => {
    ...
    const queue = tracks.map((s) => ({
      ...s,
      type: "song",
      title: s.title || s.name,
      thumbnails: s.thumbnailUrl || s.thumb,
      artists: Array.isArray(s.artists) ? s.artists : [s.artist].filter(Boolean),
    }));
